...
bool SkyDomeClass::Initialize(ID3D11Device* device)
{
	...
	result = LoadSkyDomeModel(L"./data/skydome.txt");
	if (!result)
	{
		return false;
	}
	...
	m_apexColor = XMFLOAT4(0.2f, 0.66f, 0.66f, 1.0f);

	// Set the color at the center of the sky dome.
	m_centerColor = XMFLOAT4(84.0f / 255.0f, 131.0f / 255.0f, 51.0f / 255.0f, 1.0f);

	return true;
}
// The Shutdown function releases the sky dome modeland the vertexand index buffers.
...
// GetIndexCount returns the index countand is used for rendering the sky dome.
...
// The InitializeBuffers function loads the sky dome model structure into the vertexand index buffer.
...
// The ReleaseBuffers function releases the vertexand index buffer that were used to render the sky dome.
